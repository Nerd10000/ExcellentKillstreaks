...
public class KillstreakCommand implements BasicCommand {
    @Override
    public void execute(@NotNull CommandSourceStack commandSourceStack, @NotNull String[] args) {
        ...
        if (args.length < 1){
            p.sendRichMessage(ConfigProvider.SELF_COMMAND
                    .replace("%streak%",String.valueOf(ExcellentKillStreaks.getDataManager().getStreak(p.getUniqueId().toString()))));
